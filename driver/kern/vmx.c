 ...
 * Walnut to interoperate with KVM and other HV solutions.
 *
...
#endif
static sys_call_ptr_t walnut_syscall_tbl[NUM_SYSCALLS] __cacheline_aligned;
...
/*
 * Commit 72f5e08dbba2 ("x86/entry: Remap the TSS into the CPU entry area").
 */
...
#else
static inline unsigned long segment_base(u16 selector)
{
...
static void __vmx_setup_cpu(void)
{
	 ...
	 * processors.
	 */
	vmcs_writel(HOST_TR_BASE, vmx_read_tr_base(0)); /* 22.2.4 */
 ...
 */
static void vmx_setup_initial_guest_state(struct walnut_config *conf)
{
	unsigned long tmpl;
 ...
 */
static void vmx_setup_registers(struct vmx_vcpu *vcpu, struct walnut_config *conf)
{
...
/**
 * vmx_copy_registers_to_conf - copy registers to walnut_config
 */
static void vmx_copy_registers_to_conf(struct vmx_vcpu *vcpu, struct walnut_config *conf)
{
 ...
 */
static struct vmx_vcpu * vmx_create_vcpu(struct walnut_config *conf)
{
	struct vmx_vcpu *vcpu;

	if (conf->vcpu) {
		/* This Walnut configuration already has a VCPU. */
	...
	vcpu->cpu = -1;
	vcpu->syscall_tbl = (void *) &walnut_syscall_tbl;
...
static int walnut_exit(int error_code)
{
	struct vmx_vcpu *vcpu;
	...
	vcpu->shutdown = 1;
	vcpu->ret_code = WALNUT_RET_EXIT;
	vcpu->conf->status = error_code;

	return 0;
}

static int walnut_exit_group(int error_code)
{
			 ...
			 int sysnr)
{
	...
	/*
	 * NOTE: Since Walnut processes use the kernel's LSTAR
	 ...
	 * in a child process since it is not running in Walnut.
	 * Our solution is to adopt a special Walnut convention
...
static do_fork_hack __walnut_do_fork = NULL;
static inline long
walnut_do_fork(unsigned long clone_flags, unsigned long stack_start,
	     ...
	     unsigned long tls)
{
	...
	ret = __walnut_do_fork(clone_flags, stack_start, stack_size,
...
static long walnut_sys_clone(unsigned long clone_flags, unsigned long newsp,
		...
		unsigned long tls)
{
	...
	return walnut_do_fork(clone_flags, newsp, &regs, 0, parent_tid, child_tid,
			    tls);
}
#if 0
...
static long walnut_sys_fork(void)
{
	struct vmx_vcpu *vcpu;
	struct pt_regs regs;
	...
	return walnut_do_fork(SIGCHLD, regs.sp, &regs, 0, NULL, NULL, 0);
}

static long walnut_sys_vfork(void)
{
	struct vmx_vcpu *vcpu;
	struct pt_regs regs;
	...
	return walnut_do_fork(CLONE_VFORK | CLONE_VM | SIGCHLD, regs.sp,
			    &regs, 0, NULL, NULL, 0);
}
...
static int vmx_init_syscall(void)
{
	...
	if (!syscall_table) {
		printk(KERN_ERR "Failed to lookup symbol sys_call_table");
		return -EINVAL;
	}

	memcpy(walnut_syscall_tbl, syscall_table,
	       sizeof(sys_call_ptr_t) * NUM_SYSCALLS);

	__walnut_do_fork = (do_fork_hack) kallsyms_lookup_name("_do_fork");
	if (!__walnut_do_fork) {
		printk(KERN_ERR "Failed to lookup symbol _do_fork");
		return -EINVAL;
	}
	
	walnut_syscall_tbl[__NR_exit] = (void *) &walnut_exit;
	walnut_syscall_tbl[__NR_exit_group] = (void *) &walnut_exit_group;
	walnut_syscall_tbl[__NR_clone] = (void *) &walnut_sys_clone;
	walnut_syscall_tbl[__NR_fork] = (void *) &walnut_sys_fork;
	walnut_syscall_tbl[__NR_vfork] = (void *) &walnut_sys_vfork;
...
static int vmx_handle_ept_violation(struct vmx_vcpu *vcpu)
{
	...
	if (ret) {
		printk(KERN_ERR "vmx: page fault failure "
		       "GPA: 0x%lx, GVA: 0x%lx\n",
		       gpa, gva);
		vcpu->ret_code = WALNUT_RET_EPT_VIOLATION;
		vmx_dump_cpu(vcpu);
	}
...
static int vmx_handle_nmi_exception(struct vmx_vcpu *vcpu)
{
	...
	vcpu->ret_code = WALNUT_RET_INTERRUPT;
...
/**
 * vmx_launch - the main loop for a VMX Walnut process
 * @conf: the launch configuration
 */
int vmx_launch(struct walnut_config *conf, int64_t *ret_code)
{
	int ret, done = 0;
	u32 exit_intr_info;
	...
	while (1) {
		...
		/*
		 * We assume that a Walnut process will always use
		...
		if (signal_pending(current)) {
			...
			vcpu->ret_code = WALNUT_RET_SIGNAL;
			break;
		}
		...
		if (ret == EXIT_REASON_VMCALL &&
			...
			vcpu->ret_code = WALNUT_RET_UNHANDLED_VMEXIT;
			vmx_dump_cpu(vcpu);
			done = 1;
		}
