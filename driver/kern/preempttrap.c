...
static struct walnut_trap_config trap_conf;
...
static void notifier_sched_in(struct preempt_notifier *notifier, int cpu)
{
	...
	if (trap_state.triggered && trap_state.count-- == 0) {
		...
		struct walnut_trap_regs trap_regs;
		...
		if (sizeof(struct walnut_trap_regs) == trap_conf.regs_size) {
			int r = copy_to_user((void __user *)trap_conf.regs,
				     &trap_regs, sizeof(struct walnut_trap_regs));
			if (r) {
				//TODO
				return;
			}
...
long walnut_trap_enable(unsigned long arg)
{
	unsigned long r;

	r = copy_from_user(&trap_conf, (void __user *)arg,
			 sizeof(struct walnut_trap_config));
	if (r) {
		r = -EIO;
		goto out;
	}
...
long walnut_trap_disable(unsigned long arg)
{
