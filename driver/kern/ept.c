...
static void __ept_mmu_notifier_invalidate_range_start(struct mmu_notifier *mn,
						    struct mm_struct *mm,
						    unsigned long start,
						    unsigned long end)
{
...
static int ept_mmu_notifier_invalidate_range_start(struct mmu_notifier *mn,
						   ...
						   bool blockable)
{
	/* Similar to kvm_mmu_notifier_invalidate_range_start() and
	 * kvm_arch_mmu_notifier_invalidate_range(), ignore blockable.
	 */
	__ept_mmu_notifier_invalidate_range_start(mn, mm, start, end);
	return 0;
}
#else
