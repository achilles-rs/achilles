...
static int umm_mmap_anom(void *addr, size_t len, int prot, bool big)
{
	...
	ret = lakko_vm_map_phys(pgroot, addr, len,
			      (void *) lakko_va_to_pa(addr),
			      perm);
	if (ret) {
		munmap(addr, len);
		return ret;
...
//		printf("freeing heap %lx\n", brk_len - len);
		...
		if (ret)
			return -errno;
		lakko_vm_unmap(pgroot, (void *) (mmap_base + len),
			      brk_len - len);
	} else {
...
int umm_munmap(void *addr, size_t len)
{
	...
	if (ret) {
		...
		if (ret)
			return -errno;
		lakko_vm_unmap(pgroot, addr, BIG_PGADDR(len + BIG_PGSIZE - 1));
		return 0;
	}

	lakko_vm_unmap(pgroot, addr, len);
...
int umm_mprotect(void *addr, size_t len, unsigned long prot)
{
	...
	ret = lakko_vm_mprotect(pgroot, addr, len, prot_to_perm(prot));
	assert(!ret);
...
void *umm_mremap(void *old_address, size_t old_size, size_t new_size, int flags, void *new_address)
{
	int adjust_mmap_len = 0;
	void *ret;
	...
	lakko_vm_unmap(pgroot, old_address, old_size);

        if (lakko_vm_map_phys(pgroot, new_address, new_size,
                              (void *) lakko_va_to_pa(new_address),
