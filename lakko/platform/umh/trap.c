...
static void
pgflt_handler(uintptr_t addr, uint64_t fec, struct lakko_tf *tf)
{
	int ret;
	ptent_t *pte;
	...
	if (was_user) {
		...
		print_procmap();
		lakko_ret_from_user(-EFAULT);
	} else {
		/* XXX use mem lock */
		pthread_mutex_lock(&_syscall_mtx);
		ret = lakko_vm_lookup(pgroot, (void *) addr, CREATE_NORMAL, &pte);
		assert(!ret);
		*pte = PTE_P | PTE_W | PTE_ADDR(lakko_va_to_pa((void *) addr));
...
void do_enter_thread(struct lakko_tf *tf)
{
	long rc;

	rc = lakko_jump_to_user(tf);
...
static void *pthread_entry(void *arg)
{
	...
	struct lakko_tf *tf = a->ta_tf;
	struct lakko_tf child_tf;
	int *tidp = NULL;
	pid_t tid;
	int flags = ARG0(tf);
...
static long lakko_pthread_create(struct lakko_tf *tf)
{
	pthread_t pt;
...
static long lakko_clone(struct lakko_tf *tf)
{
	unsigned long fs;
	int rc;
	///unsigned long pc;
	...
	if (ARG1(tf) != 0)
		return lakko_pthread_create(tf);

	fs = lakko_get_user_fs();
...
static int syscall_check_params(struct lakko_tf *tf)
{
	void *ptr = NULL;
	uint64_t len = 0;
	char *str = NULL;
	...
	case SYS_open:
		input = 4;
		output = place_holder(input);

		printf("find SYS_OPEN, input = %d output = %d\n", input, output);
...
static int syscall_allow(struct lakko_tf *tf)
{
	if (!_syscall_monitor) {
		tf->rax = -EPERM;
		return 0;
...
static void syscall_do_foreal(struct lakko_tf *tf)
{
	switch (tf->rax) {
	case SYS_arch_prctl:
		switch (ARG0(tf)) {
		case ARCH_GET_FS:
			*((unsigned long*) ARG1(tf)) = lakko_get_user_fs();
			tf->rax = 0;
			break;

		case ARCH_SET_FS:
			lakko_set_user_fs(ARG1(tf));
			tf->rax = 0;
			break;
	...
	case SYS_exit_group:
	case SYS_exit:
		lakko_ret_from_user(ARG0(tf));
		break;

	default:
		lakko_passthrough_syscall(tf);
		break;
	}
}

static void syscall_do(struct lakko_tf *tf)
{
	int need_lock = 0;
...
static void syscall_handler(struct lakko_tf *tf)
{
...
int trap_init(void)
{
	...
	lakko_register_pgflt_handler(pgflt_handler);
	lakko_register_syscall_handler(&syscall_handler);
