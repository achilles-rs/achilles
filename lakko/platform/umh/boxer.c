...
static int process_elf_ph(struct lakko_elf *elf, Elf64_Phdr *phdr)
{
	int ret;
	int perm = PERM_U;
	off_t off;
	...
	ret = lakko_elf_load_ph(elf, phdr, off);
	if (ret) {
	...
	ret = lakko_vm_mprotect(pgroot, (void *) (phdr->p_vaddr + off),
			      phdr->p_memsz, perm);
	if (ret) {
...
{
	int ret;
	struct lakko_elf elf;

	ret = lakko_elf_open(&elf, path);
	if (ret)
		return ret;
	...
	ret = lakko_elf_iter_ph(&elf, &process_elf_ph);
	if (ret)
		goto out;
...
static int run_app(uintptr_t sp, uintptr_t e_entry)
{
	...
	memset(&tf, 0, sizeof(struct lakko_tf));
	tf.rip = e_entry;
	tf.rsp = sp;
	tf.rflags = 0x0;

	return lakko_jump_to_user(&tf);
}
...
//	printf("%s\n", environ[0]);
	...
	ret = lakko_init(0);
	if (ret) {
		printf("sandbox: failed to initialize Lakko\n");
		return ret;
	}

	ret = lakko_enter();
	if (ret) {
		printf("sandbox: failed to enter Lakko mode\n");
		return ret;
	}
...
//	printf("sandbox: entry addr is %lx\n", data.entry);
	...
	lakko_set_user_fs(0); // default starting fs
