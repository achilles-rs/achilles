/*
 * fpu.h - x86 floating point, MMX, SSE, and AVX support for Walnut
 */
...
/*
 * lakko_fpu_init - initializes an fpu area
 * @fp: the fpu area
 */
static inline void lakko_fpu_init(struct fpu_area *fp)
{
...
/*
 * lakko_fpu_load - loads an fpu area into fpu registers
 * @fp: the fpu area
 */
static inline void lakko_fpu_load(struct fpu_area *fp)
{
	fpu_xrstor(fp, -1);
}

/*
 * lakko_fpu_save - saves fpu registers to an fpu area
 * @fp: the fpu area
 * 
 * WARNING: Do not call this function on a memory region
 * that was not previously loaded with lakko_fpu_load().
 * 
 ...
 * for more details.
 */
static inline void lakko_fpu_save(struct fpu_area *fp)
{
...
/*
 * lakko_fpu_save_safe - saves an fpu area from CPU registers
 * @fp: the fpu area
 * 
 * Works under all conditions, but may be slower.
 */
static inline void lakko_fpu_save_safe(struct fpu_area *fp)
{
	fpu_xsave(fp, -1);
}
