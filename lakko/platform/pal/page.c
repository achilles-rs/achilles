...
void walnut_page_stats(void)
{
	...
	walnut_printf("WALNUT Page Allocator: Alloc %d, Free %d, Total %d\n",
			   num_alloc, num_pages - num_alloc, num_pages);
}

struct page * walnut_page_alloc(void)
{
	struct page *pg;
...
void walnut_page_free(struct page *pg)
{
	assert(!pg->ref);
	pthread_mutex_lock(&page_mutex);
...
bool lakko_page_isfrompool(physaddr_t pa)
{
	// XXX: Insufficent?
	return (pa >= PAGEBASE) && (pa < PAGEBASE + num_pages*PGSIZE);
}

int walnut_page_init(void)
{
	int i;
	void *mem;
