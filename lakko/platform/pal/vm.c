...
{
	struct page *pg = walnut_page_alloc();
	if (!pg)
		return NULL;

	return (void *) lakko_page2pa(pg);
}

static inline void put_page(void * page)
{
	// XXX: Using PA == VA
	struct page *pg = lakko_pa2page((physaddr_t)page);

	lakko_page_put(pg);
}

int __lakko_vm_page_walk(ptent_t *dir, void *start_va, void *end_va,
			...
			int create)
{
	...
	for (i = start_idx; i <= end_idx; i++) {
		...
		ret = __lakko_vm_page_walk((ptent_t *) PTE_ADDR(dir[i]),
...
int lakko_vm_page_walk(ptent_t *root, void *start_va, void *end_va,
		     page_walk_cb cb, const void *arg)
{
	return __lakko_vm_page_walk(root, start_va, end_va, cb, arg, 3, CREATE_NONE);
}

 int lakko_vm_lookup(ptent_t *root, void *va, int create, ptent_t **pte_out)
{
	// XXX: Using PA == VA
	int i, j, k, l;
...
static int __lakko_vm_mprotect_helper(const void *arg, ptent_t *pte, void *va)
{
...
int lakko_vm_mprotect(ptent_t *root, void *va, size_t len, int perm)
{
	int ret;
	ptent_t pte_perm;
	...
	ret = __lakko_vm_page_walk(root, va, va + len - 1,
				 &__lakko_vm_mprotect_helper,
...
static int __lakko_vm_map_phys_helper(const void *arg, ptent_t *pte, void *va)
{
...
int lakko_vm_map_phys(ptent_t *root, void *va, size_t len, void *pa, int perm)
{
	int ret;
...
static int __lakko_vm_map_pages_helper(const void *arg, ptent_t *pte, void *va)
{
	ptent_t perm = (ptent_t) arg;
	struct page *pg = walnut_page_alloc();
	...
	*pte = PTE_ADDR(lakko_page2pa(pg)) | perm;

	return 0;
}

int lakko_vm_map_pages(ptent_t *root, void *va, size_t len, int perm)
{
	int ret;
	ptent_t pte_perm;
	...
	ret = __lakko_vm_page_walk(root, va, va + len - 1,
				 &__lakko_vm_map_pages_helper,
				 (void *) pte_perm, 3, CREATE_NORMAL);

	return ret;
}

static int __lakko_vm_clone_helper(const void *arg, ptent_t *pte, void *va)
{
       int ret;
       struct page *pg = lakko_pa2page(PTE_ADDR(*pte));
       ptent_t *newRoot = (ptent_t *)arg;
       ptent_t *newPte;

       ret = lakko_vm_lookup(newRoot, va, CREATE_NORMAL, &newPte);
       if (ret < 0)
               return ret;

       if (lakko_page_isfrompool(PTE_ADDR(*pte)))
               walnut_page_get(pg);
       *newPte = *pte;
 ...
 * Clone a page root.
 */
ptent_t *lakko_vm_clone(ptent_t *root)
{
       int ret;
       ptent_t *newRoot;
       ...
       ret = __lakko_vm_page_walk(root, VA_START, VA_END,
                       &__lakko_vm_clone_helper, newRoot,
                       3, CREATE_NONE);
       if (ret < 0) {
               lakko_vm_free(newRoot);
               return NULL;
       }
...
static int __lakko_vm_free_helper(const void *arg, ptent_t *pte, void *va)
{
	struct page *pg = lakko_pa2page(PTE_ADDR(*pte));
 ...
 */
void lakko_vm_free(ptent_t *root)
{
	// XXX: Should only need one page walk
	// XXX: Hacky - Until I fix ref counting
	/*__lakko_vm_page_walk(root, VA_START, VA_END,
			&__lakko_vm_free_helper, NULL,
			3, CREATE_NONE);*/
...
void lakko_vm_unmap(ptent_t *root, void *va, size_t len)
{
	/* FIXME: Doesn't free as much memory as it could */
	__lakko_vm_page_walk(root, va, va + len - 1,
			&__lakko_vm_free_helper, NULL,
			3, CREATE_NONE);
...
void lakko_vm_default_pgflt_handler(uintptr_t addr, uint64_t fec)
{
	ptent_t *pte = NULL;
	int rc;
	...
	rc = lakko_vm_lookup(pgroot, (void *) addr, 0, &pte);
	assert(rc == 0);
	...
	if ((fec & FEC_W) && (*pte & PTE_COW)) {
		...
		if (lakko_page_isfrompool(PTE_ADDR(*pte)) && pg->ref == 1) {
