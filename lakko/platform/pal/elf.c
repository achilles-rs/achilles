...
static int elf_read(struct lakko_elf *elf, void *dst, int len, int off)
{
	int rd;
...
/**
 * lakko_elf_load_ph - a utility function to map a .LOAD region into program memory
 * @elf: elf data
 * @phdr: the program header
 * 
 * Returns 0 on success, otherwise failure.
 */
int lakko_elf_load_ph(struct lakko_elf *elf, Elf64_Phdr *phdr, off_t off)
{
	void *ptr;
	int prot = PROT_NONE;
...
static int elf_open_phs(struct lakko_elf *elf)
{
	int ret;
	size_t len;
	Elf64_Phdr *phdr;
...
/**
 * lakko_elf_iter_ph - iterates over each program header
 * @elf: elf data
 ...
 * value.
 */
int lakko_elf_iter_ph(struct lakko_elf *elf, lakko_elf_phcb cb)
{
	int i, ret;
...
static int elf_open_shs(struct lakko_elf *elf)
{
	int ret;
	size_t len;
	Elf64_Shdr *shdr;
...
/**
 * lakko_elf_iter_sh - iterates over each section header
 * @elf: elf data
 ...
 * value.
 */
int lakko_elf_iter_sh(struct lakko_elf *elf, lakko_elf_shcb cb)
{
	int i, ret;
	...
	for (i = 0; i < elf->hdr.e_shnum; i++) {
		...
		if (strnlen(sname, MAX_SNAME) == MAX_SNAME) {
			walnut_printf("elf: section name too long\n");
			return -EINVAL;
		}
...
static int do_elf_open(struct lakko_elf *elf)
{
	Elf64_Ehdr hdr;
	int ret;
...
/**
 * lakko_elf_open - Open an elf binary
 * @elf: lakko_elf object
 * @path: file path to the elf object
 *
 * Returns: 0 on success, otherwise failure.
 */
int lakko_elf_open(struct lakko_elf *elf, const char *path)
{
	int fd;
...
int lakko_elf_open_mem(struct lakko_elf *elf, void *mem, int len)
{
	elf->fd  = -1;
	elf->mem = mem;
	elf->len = len;
...
/**
 * lakko_elf_close - Close an elf object.
 * @elf: lakko_elf object
 */
int lakko_elf_close(struct lakko_elf *elf)
{
	if (elf->phdr != NULL) {
		free(elf->phdr);
		elf->phdr = NULL;
...
static int elf_dump_sh(struct lakko_elf *elf,
		       const char *sname,
		       int snum,
		       Elf64_Shdr *shdr)
{
	...
	walnut_printf("  [%2d] %-16s %-16s %016llx %08x\n",
			   snum, sname, type, shdr->sh_addr, shdr->sh_offset);
	walnut_printf("       %016llx %016llx %c%c%c%c%c %4d %4d %5d\n",
...
static int elf_dump_ph(struct lakko_elf *elf, Elf64_Phdr *phdr)
{
	const char *type = "UNKNOWN";
	...
	walnut_printf("  %-20s 0x%016llx 0x%016llx 0x%016llx\n",
			   type, phdr->p_offset,
			   phdr->p_vaddr, phdr->p_paddr);
	walnut_printf("  %-20s 0x%016llx 0x%016llx  %c%c%c    %x\n",
...
/**
 * lakko_elf_dump - Dumps an elf binary program headers and segments
 * @elf: lakko_elf object
 * 
 * Returns: 0 on success, otherwise failure.
 */
int lakko_elf_dump(struct lakko_elf *elf)
{
	int ret = 0;

	walnut_printf("--- ELF Dump ---\n");

	// XXX: Dump Header

	walnut_printf("Section Headers:\n");
	walnut_printf("  [Nr] %-16s %-16s %-16s %s\n",
			   "Name", "Type", "Address", "Offset");
	walnut_printf("       %-16s %-16s %s %s %s  %s\n",
			   "Size", "EntSize", "Flags", "Link", "Info", "Align");
	if ((ret = lakko_elf_iter_sh(elf, &elf_dump_sh))) {
		printf("elf: failed to dump section headers\n");
		goto out;
	}

	walnut_printf("Program Headers:\n");
	walnut_printf("  %-20s %-18s %-18s %s\n",
			   "Type", "Offset", "VirtAddr", "PhysAddr");
	walnut_printf("  %-20s %-18s %-18s %s   %s\n",
			   "", "FileSiz", "MemSize", "Flags", "Align");
	if ((ret = lakko_elf_iter_ph(elf, &elf_dump_ph))) {
